...
export default function CreatePatientForm({ onClose, setNotification}) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setNotification?.({ message: "Error al guardar el paciente", type: "error" });

      if (error.response?.data?.message) {
        alert(error.response.data.message);
      } else {
        setNotification?.({ message: "Error de conexión con el servidor. Intente nuevamente.", type: "error" });
      }
    }
